...
public class Node{
	...
	public ArrayList<int[][]> expand()
	{
		...
		for(int i = 1 ; i <= 8; i++) {
			...
			if(x - 1 >= 0) nodeslist.add(moveBlankUp(x, y));
			if(x + 1 < board.length) nodeslist.add(moveBlankDown(x,y));
			if(y - 1 >= 0) nodeslist.add(moveBlankLeft(x,y));
			if(y + 1 < board.length) nodeslist.add(moveBlankRight(x, y));
		}
    ...
    //Return the f value as the sum of g and h value
   ...
   public double evaluateHeuristic() {
         ...
         */
         ...
         for(int i=0; i<size; i++){
           for(int j=0; j<size; j++){
        	 //Note that we won't count 0 because 0 does not count as a tile
             if(board[i][j] != goal[i][j] && board[i][j]!=0){
               int r = getGoalRow(board[i][j]);
               int c = getGoalCol(board[i][j]);
               heuristic += getManhattanDistance(r,c,i,j);
             }
           }
         }
         return heuristic;
   ...
   /* My own heuristic that calculate the number of misplaced tile through both row and column
    * 
    */
   public double evaluateMyHeuristic() {
       ...
       for(int i=0; i<size; i++){
         for(int j=0; j<size; j++){
           if(i!=getGoalRow(board[i][j]) && board[i][j]!=0) {
        	   heuristic++;
           }
           if(j!=getGoalCol(board[i][j]) && board[i][j]!=0) {
        	   heuristic++;
           }
         }
       }
...
//Return the goal row number of a given misplaced tile
    private int getGoalRow(int misplaced){
      for(int i=0; i<size; i++){
        for(int j=0; j<size; j++){
          if(misplaced == goal[i][j]){
            return i;
          }
        }
      }
...
//Return the goal column number of a given misplaced tile
    private int getGoalCol(int misplaced){
      for(int i=0; i<size; i++){
        for(int j=0; j<size; j++){
          if(misplaced == goal[i][j]){
            return j;
